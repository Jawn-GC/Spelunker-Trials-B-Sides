meta = {
    name = 'Spelunker Trials: B-Sides',
    version = '1.0',
    description = 'Remixed ideas from the Spelunker Trials series (plus new stuff)',
    author = 'JawnGC',
}
...
local kaizo_blocks = require('kaizo_block')
...
replace_drop(DROP.EGGSAC_GRUB_1, ENT_TYPE.ITEM_NUGGET_SMALL)
replace_drop(DROP.EGGSAC_GRUB_2, ENT_TYPE.ITEM_NUGGET_SMALL)
replace_drop(DROP.EGGSAC_GRUB_3, ENT_TYPE.ITEM_NUGGET_SMALL)
...
set_callback(function()
	...
	elseif level_sequence.get_run_state().current_level.identifier == "Tidepool-3" then
		state.level_flags = set_flag(state.level_flags, 18)
	else	
		state.level_flags = clr_flag(state.level_flags, 18)
	end	
